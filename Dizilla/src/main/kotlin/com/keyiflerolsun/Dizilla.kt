...
class Dizilla : MainAPI() {
    ...
    override val mainPage = mainPageOf(
        ...
        "${mainUrl}/arsiv" to "Yensi Eklenen Diziler",
                ...
                try {
                    ...
                    if (secureData != null) {
                        ...
                        if (decodedData != null) {
                            ...
                            if (relatedResults != null) {
                                // Yeni eklenen bölümler için getLatestEpisodes endpoint'ini kullan
                                val latestEpisodes = relatedResults.get("getLatestEpisodes") 
                                    ?: relatedResults.get("getDiscoverArchive")
                                
                                if (latestEpisodes != null) {
                                    val resultArray = latestEpisodes.get("result")
                                    ...
                                    if (resultArray != null && resultArray.isArray) {
                                        val jsonResults = resultArray.mapNotNull {
                                            ...
                                            // Eğer sezon ve bölüm bilgisi varsa ekle
                                            val season = it.get("season")?.asInt()
                                            val episode = it.get("episode")?.asInt()
                                            ...
                                            newTvSeriesSearchResponse(episodeTitle, fixUrl("/$slug"), TvType.TvSeries) {
                                                this.posterUrl = poster
                                            }
                                        }
                    ...
                    Log.e("Dizilla", "JSON parsing failed for tum-bolumler: ${e.message}")
                }
            }
            
            // JSON parsing başarısız olduysa HTML parsing kullan
            document.select("div.grid.sm\\:grid-cols-3 a, div.col-span-3 > div.grid a").mapNotNull { it.sonBolumler() }
        }
    ...
    private suspend fun Element.sonBolumler(): SearchResponse? {
        // HTML'deki yapıya göre: <a href="/dizi-2-sezon-3-bolum">
        val hrefAttr = this.attr("href")
        val href = fixUrlNull(hrefAttr) ?: return null
        
        // Dizi adını bul
        val name = this.selectFirst("h2")?.text()?.trim() ?: return null
        
        // Sezon ve bölüm bilgisini bul
        val seasonEpisodeDiv = this.selectFirst("div.opacity-80")
        val epName = if (seasonEpisodeDiv != null) {
            seasonEpisodeDiv.text()
                .replace(". Sezon ", "x")
                .replace(". Bölüm", "")
                .replace(Regex("\\s+"), "")
                .trim()
        } else {
            // href'ten çıkar: /dizi-2-sezon-3-bolum -> 2x3
            val match = Regex("(\\d+)-sezon-(\\d+)-bolum").find(hrefAttr)
            if (match != null) {
        ...
        val title = if (epName.isNotEmpty()) {
            "$name - $epName"
        } else {
            name
        }
        ...
        val posterUrl = fixUrlNull(
            ...
            this.selectFirst("img[data-nimg]")?.attr("src")
        )
