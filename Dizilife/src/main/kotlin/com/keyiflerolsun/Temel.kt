...
class Temel : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        return if (isTvSeries) {
            // Dizi için bölüm listesi
            val episodes = document.select("div.episode-item, div.episode, a[href*='/bolum'], a[href*='/episode']").mapNotNull {
                val epHref = fixUrlNull(it.attr("href") ?: it.attr("data-url")) ?: return@mapNotNull null
                val epName = it.text().trim().takeIf { it.isNotEmpty() } 
                    ?: it.selectFirst("span, div")?.text()?.trim()
                    ?: "Bölüm"
